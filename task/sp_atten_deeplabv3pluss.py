...
# 以densenet为基础，可以自定义其网络结果层数，模块数。
class DenseNet(object):
    """
    以densenet为基础，可以自定义其网络结果层数，模块数。包括stage2-stage5的瓶颈块的格式，生长率，压缩率
    params, growth_rate, reduction
    """
...
class DeepLabV3Plus():
    """
    骨干网络为denset类型,即encoder编码器，后面分支网络则为deeplabv3+，主要构成是aspp + 一个中间上采样层
    """
    ...
    def _aspp(self, x, out_filters):
        ...
        x1 = sp_block_1(x1) # Spatial attention
        # x1 = sp_block_2(x1) # Spatial attention

        xs.append(x1)

        for i in range(3):
            xi = layers.Conv2D(out_filters, 3, strides=1, padding='same',kernel_initializer='he_normal', dilation_rate=6 * (i + 1))(x)
            xi = sp_block_1(xi)  # Spatial attention
            # xi = sp_block_2(xi) # Spatial attention
            xs.append(xi)
        ...
        img_pool = sp_block_1(img_pool)  # Spatial attention
        # img_pool = sp_block_2(img_pool) # Spatial attention
